      ...
      else
      {
        switch(vStatusCode)
        {
          case -1: // Not Available (OK for readystates: MSXML<4=1-3, MSXML>3=1-2, Gecko=1)
            if (qx.core.Variant.isSet("qx.debug", "on"))
            {
              if (qx.core.Setting.get("qx.ioRemoteDebug") && vReadyState > 3) {
                qx.log.Logger.debug("Failed with statuscode: -1 at readyState " + vReadyState);
              }
            }
          ...
          case 206: // Partial Content
            if (qx.core.Variant.isSet("qx.debug", "on"))
            {
              if (qx.core.Setting.get("qx.ioRemoteDebug") && vReadyState === 4) {
                qx.log.Logger.debug("Failed with statuscode: 206 (Partial content while being complete!)");
              }
            }
          ...
          case 505: // HTTP Version not supported
            if (qx.core.Variant.isSet("qx.debug", "on"))
            {
              if (qx.core.Setting.get("qx.ioRemoteDebug")) {
                qx.log.Logger.debug("Failed with typical HTTP statuscode: " + vStatusCode);
              }
            }
          ...
          case 13030:
            ...
            {
              if (qx.core.Setting.get("qx.ioRemoteDebug")) {
                qx.log.Logger.debug("Failed with MSHTML specific HTTP statuscode: " + vStatusCode);
              }
            }
          ...
          default:
            ...
            qx.log.Logger.debug("Unknown status code: " + vStatusCode + " (" + vReadyState + ")");
            return false;
        }
      }
    },
