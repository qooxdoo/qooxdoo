 ...
 */
qx.Class.textToHtml = function(str) {
  return qx.util.StringEscape.escapeHtml(str).replace(/(  |\n)/g, function(chr) {
    var map = {
      "  ": " &nbsp;",
      "\n": "<br>"
    }
 ...
 */
qx.Class.htmlToText = function(str) {
  return qx.util.StringEscape.unescapeHtml(str.replace(/\s+|<([^>])+>/gi, function(chr) {
    if (/\s+/.test(chr)) {
      return " ";
    }
