{
  "info" : 
  {
    "name" : "DemoBrowser Samples",

    "summary" : "Sample applications that run in the DemoBrowser",
    "description" : "Sample applications that run stand-alone and in the DemoBrowser",
    
    "keywords" : ["demos", "samples"],
    "homepage" : "http://demo.qooxdoo.org/current/demobrowser",

    "license" : "LGPL/EPL",
    "authors" : 
    [
      {
        "name" : "Sebastian Werner (wbasti)",
        "email" : "info@sebastian-werner.net"
      },
      {
        "name" : "Fabian Jakobs (fjakobs)",
        "email" : "fabian.jakobs@1und1.de"
      }
    ],

    "version" : "trunk",
    "qooxdoo-versions": ["trunk"]
  },
  
  "provides" : 
  {
    "namespace"   : "demobrowser.demo",
    "encoding"    : "utf-8",
    "class"       : "source/class",
    "resource"    : "source/resource",
    "translation" : "source/translation",
    "type"        : "application"
  }
}

