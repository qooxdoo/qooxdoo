...
def main():
  ...
  while i < len(sys.argv):
    ...
    if c == "-sd" or c == "--source-directories":
      sourceDirectories = sys.argv[i].split(",")
    ...
    elif c == "-osx" or c == "--output-syntax-xml":
      outputSyntaxXml = sys.argv[i]
      i += 1

    elif c == "-odx" or c == "--output-doc-xml":
      outputDocXml = sys.argv[i]
      i += 1

    elif c == "-odj" or c == "--output-doc-json":
      outputDocJson = sys.argv[i]
      i += 1
  ...
  print "  * Output doc JSON (-odj):   %s" % outputDocJson

  docTree = None
  for basedir in sourceDirectories:
        ...
        if filename.endswith(".js"):
          ...
          if (onlyClassesRe == None or onlyClassesRe.search(className)) and (skipClassesRe == None or not skipClassesRe.search(className)):
            ...
            if outputTokenJson:
              ...
              outfilename = os.path.join(outputTokenJson,className + ".js")
  ...
  if docTree:
    ...
    postWorkPackage(docTree, docTree)
