...
qx.Bootstrap.define("qx.event.handler.GestureCore", {
  ...
  statics : {
     ...
     */
    TOUCHPAD_WHEEL_EVENTS_TIMEOUT: 500
  },
  ...
  construct : function(target, emitter) {
    ...
    this.__isTouchPadRoll= false;
    this.__rollEventCount = 0;
    this.__rollEventCountStart = 0;
  ...
  members : {
    ...
    __isTouchPadRoll: null,
    __rollEventCount : null,
    __rollEventCountStart : null,
    __touchPadDetectionPerformed : null,
    __lastRollEventTime: null,
    ...
    */
    ...
    _fireRoll : function(domEvent, type, target) {
      if (domEvent.type === qx.core.Environment.get("event.mousewheel").type) {
        if (this.__lastRollEventTime > 0 && new Date().getTime() - this.__lastRollEventTime >
            qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_TIMEOUT) {
          // The timeout was reached. A new detection step should occur.
          this.__touchPadDetectionPerformed = false;
          this.__rollEventCount = 0;
          this.__lastRollEventTime = 0;
        }
        if (!this.__touchPadDetectionPerformed) {
          ...
          // de tection period is over.
          if (this.__rollEventCount === 0) {
            this.__rollEventCountStart = new Date().getTime();
          }
          this.__rollEventCount++;
          if (new Date().getTime() - this.__rollEventCountStart >
              qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD) {
            if (this.__rollEventCount > qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_THRESHOLD) {
              this.__isTouchPadRoll = true;
            } else {
              this.__isTouchPadRoll = false;
            }
          ...
          var rollFactor = qx.event.handler.GestureCore.WHEEL_ROLL_FACTOR;
          if (this.__isTouchPadRoll) {
            // The event was generated by a touchpad
