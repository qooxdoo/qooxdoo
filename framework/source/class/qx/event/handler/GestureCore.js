...
qx.Bootstrap.define("qx.event.handler.GestureCore", {
  ...
  construct : function(target, emitter) {
    ...
    this.__rollEvents = [];
    this.__rollEventsCount = 0;
    this.__rollEventsCountStart = 0;
  ...
  members : {
    ...
    __rollEvents : null,
    __rollEventsCountStart : null,
    __rollEventsCount : null,
    ...
    _fireRoll : function(domEvent, type, target) {
      if (domEvent.type === qx.core.Environment.get("event.mousewheel").type) {
        if (this._isTouchPadDetectionEnabled()) {
          ...
          if (this.__lastRollEventTime > 0 && now - this.__lastRollEventTime
          ...
          if (this.__touchPadDetectionPerformed) {
            ...
            var event = this.__performAdaptativeRollScrolling(target);
          }
        } else {
          this.__fireRollEvent(domEvent, target, qx.event.handler.GestureCore.WHEEL_ROLL_FACTOR);
        }
      } else {
        var gesture = this.__gesture[domEvent.pointerId];
        domEvent.delta = {
