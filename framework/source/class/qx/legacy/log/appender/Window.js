  ...
  members :
  {
    ...
    _openWindowCallback : function()
    {
      ...
      logDocument.write("<html><head><title>" + this._name + "</title></head>"
        + '<body onload="qx = opener.qx;" onunload="try{qx.legacy.log.WindowAppender._registeredAppenders[' + this._id + ']._autoCloseWindow()}catch(ex){}">'
        + '  <style type="text/css">'
    ...
    appendLogEvent : function(evt)
    {
      ...
      else
      {
        ...
        if (evt.isDummyEventForMessage) {
          ...
          txt = qx.bom.String.fromText(this.formatLogEvent(evt));
        }
