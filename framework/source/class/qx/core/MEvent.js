...
qx.Mixin.define("qx.core.MEvent",
{
  members :
  {
    ...
    /**
     * Creates and dispatches an event on this object, with support for
     ...
     * @param args {Array?null} Arguments, which will be passed to
     *       the event's init method.
     * @return {qx.Promise} resolved when the event has finished firing
     */
    fireEventAsync : qx.core.Environment.select("qx.promise", {
      "true": function(type, clazz, args) {
        if (!this.$$disposed) {
          return this.__Registration.fireEventAsync(this, type, clazz, args);
        }
  
        return qx.Promise.resolve(null);
      },
      "false": function(type, clazz, args) {
    ...
    /**
     * Create an event object and dispatch it on this object, with support for
     * async event handlers.
     * 
     * The event dispatched with this method does never bubble! Use only if you
     * are sure that bubbling is not required.
    ...
    fireNonBubblingEventAsync : qx.core.Environment.select("qx.promise", {
      ...
      "false": function(type, clazz, args) {
        return this.fireNonBubblingEvent(type, clazz, args);
      }
    }),
    ...
    /**
     * Creates and dispatches an non-bubbling data event on this object, with
     * support for listeners that support promises
     *
     * @param type {String} Event type to fire
     * @param data {var} User defined data attached to the event object
     * @param oldData {var?null} The event's old data (optional)
     ...
     * @return {qx.Promise} resolved when the event has finished firing
     */
    fireDataEventAsync : qx.core.Environment.select("qx.promise", {
      "true": function(type, data, oldData, cancelable) {
        if (!this.$$disposed) {
          if (oldData === undefined) {
            oldData = null;
          }
      ...
      "false": function(type, data, oldData, cancelable) {
        return this.fireDataEvent(type, data, oldData, cancelable);
      }
    })
  }
});
