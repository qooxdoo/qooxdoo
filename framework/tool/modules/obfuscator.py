...
qooxdooStart = [
    # Properties
    "_modify", "_check", "_unitDetection", "_value",
...
qooxdooNames = [
    ...
    # Inheritance
    "qx", "Proto", "Class", "Clazz", "addProperty", "removeProperty", "changeProperty", "addFastProperty", "addCachedProperty", "addPropertyGroup"
]
...
systemNames = [
    ...
    # CSS border styles
    "solid", "dotted", "double", "groove", "ridge", "outset", "inset", "none",
    
    # CSS core values
    "0px", "100%", "auto", "none"
]

def search(node, identifiers, verbose):
    if node.type == "keyvalue":
        add(node.get("key", False), identifiers)
        
    elif node.type == "definition":
        add(node.get("identifier", False), identifiers)
        ...
        if left != None:
            ...
            if var != None:
                ...
                if last != None and last.type == "identifier":
                    add(last.get("name", False), identifiers)

    if node.hasChildren():
        for child in node.children:
            search(child, identifiers, verbose)
...
def add(key, identifiers):
    ...
    if not identifiers.has_key(key):
        ...
        #identifiers[key] = "$$OBFUS_%s" % key

def patch(node, identifiers, verbose):
    if hasattr(node, "obfProtect"):
        return
    
    if node.type == "keyvalue":
        key = node.get("key")
        if key != None and identifiers.has_key(key):
            #print "        - Replace key: %s with %s (keyvalue)" % (key, identifiers[key])
            node.set("key", identifiers[key])
            
    elif node.type == "identifier":
        key = node.get("name", False) 
        if key != None and identifiers.has_key(key):
            #print "        - Replace key: %s with %s (identifier)" % (key, identifiers[key])
            node.set("name", identifiers[key])
            
    elif node.type == "definition":
        key = node.get("identifier", False) 
        if key != None and identifiers.has_key(key):
            #print "        - Replace key: %s with %s (definition)" % (key, identifiers[key])
            node.set("identifier", identifiers[key])
            
    elif node.type == "constant" and node.get("constantType") == "string":
        oldstr = node.get("value")
        new = []
        for part in oldstr.split("."):
            if identifiers.has_key(part):
                part = identifiers[part]
            new.append(part)
                
        newstr = u".".join(new)
        if oldstr != newstr:
            # print "Apply: %s -> %s" % (oldstr, newstr)
            node.set("value", newstr)
    
    # Protect strings with property names
    elif node.type == "call":
        ...
        if oper:
            ...
            if var:
                ...
                if last and last.type == "identifier" and last.get("name") in ["addProperty", "removeProperty", "changeProperty", "addFastProperty", "addCachedProperty", "addPropertyGroup"]:
                    config = node.getChild("params").getChild("map", False)
                    if config:
                        for prop in config.children:
                            if prop.type == "keyvalue" and prop.get("key") == "name":
                                prop.obfProtect = True
                    
    if node.hasChildren():
        for child in node.children:
            patch(child, identifiers, verbose)
