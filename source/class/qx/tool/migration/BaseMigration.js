  ...
  members: {
        ...
        if (dryRun) {
          ...
          for (let [newPath, oldPath] of filesToRename) {
            try {
              ...
              throw new qx.tool.utils.Utils.UserError(`Renaming '${oldPath}' to '${newPath}' failed: ${e.message}.`);
            }
          }
          this.markAsApplied();
        }
      ...
      for (let replaceInFiles of replaceInFilesArr) {
        if (
          ...
          try {
            ...
            throw new qx.tool.utils.Utils.UserError(`Error replacing in files: ${e.message}`);
          }
        }
      }
    },
    ...
    async upgradePackagesUnlessDryRun() {
      ...
      if (runner.getDryRun()) {
        ...
        await new qx.tool.compiler.cli.commands.package.Upgrade().process(options);
        this.markAsApplied();
      }
    }
  }
