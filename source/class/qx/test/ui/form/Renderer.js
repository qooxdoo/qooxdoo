...
qx.Class.define("qx.test.ui.form.Renderer", {
  ...
  members: {
    ...
    __testBindings(clazz) {
      ...
      this.assertEquals(
        2,
        text.getBindings().length,
        "Text field should have one binding!"
      );
      ...
      this.assertEquals(
        ...
        "Label should have one binding!"
      );

      // text field and label bindings must be equal
      this.assertTrue(
        qx.lang.Array.equals(text.getBindings(), label.getBindings()),
        "Text field and label bindings must be equal"
      );
