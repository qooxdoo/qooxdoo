...
qx.Class.define("qx.ui.list.column.AbstractColumn", {
  ...
  members: {
    ...
    getHeaderWidget() {
      ...
      if (!widget) {
        widget = this.__headerWidget = new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({
          appearance: "virtual-list-header-cell"
        });
        widget.add(new qx.ui.basic.Label(this.getCaption()).set({ rich: true, wrap: true, allowGrowX: true, allowGrowY: true }));
      }
      return widget;
    },
    ...
    updateEditable() {
      ...
      Object.keys(this.__bindData).forEach(hash => this._updateEditableImpl(this.__bindData[hash], enabled, readOnly));
    },
    ...
    _bindSingleSelection(widget, model, path, options) {
      ...
      let bindData = {
          model,
          widget,
          modelValueBindId: model.addListener("change" + firstUp, evt => updateWidget(evt.getData(), true)),
    ...
    _unbindSingleSelection(bindData) {
      ...
      bindData.widget.getModelSelection().removeListenerById(bindData.widgetBindId);
    },
    ...
    _createCellWidget(rowModel) {
      throw new Error("No such implementation for " + this.classname + "._createCellWidget");
    },
    ...
    _bindCellWidget(widget, rowModel) {
      throw new Error("No such implementation for " + this.classname + "._bindCellWidget");
    },
    ...
    _unbindCellWidget(widget, bindData) {
      throw new Error("No such implementation for " + this.classname + "._unbindCellWidget");
    },
    ...
    _updateEditableImpl(bindData, enabled, readOnly) {
      throw new Error("No such implementation for " + this.classname + "._updateEditableImpl");
    },
    ...
    _applyMinWidth(value) {
      let list = this.getList();
      if (list)
        list.getPane().getColumnConfig().setItemMinSize(this.getColumnIndex(), value);
    },
    ...
    _applyMaxWidth(value) {
      let list = this.getList();
      if (list)
        list.getPane().getColumnConfig().setItemMaxSize(this.getColumnIndex(), value);
    },
