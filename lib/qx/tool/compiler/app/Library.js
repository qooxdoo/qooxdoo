...
//        value = path.resolve(value);
    ...
    loadManifest: function(rootDir, cb) {
      var t = this;
      qx.tool.compiler.files.Utils.correctCase(rootDir)
        ...
        .then(data => {
          ...
          function fixLibraryPath(dir) {
            return qx.tool.compiler.files.Utils.correctCase(path.resolve(rootDir, dir))
              .then(dir => {
                if (dir.substring(0, rootDir.length + 1) != rootDir + "/")
                  t.warn("Unable to correct case for library in " + rootDir + " because it uses source/resource directories which are outside the library");
                else
