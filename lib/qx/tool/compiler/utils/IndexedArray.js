 ...
 */
qx.Class.define("qx.tool.compiler.utils.IndexedArray", {
  ...
  members: {
    ...
    sort: function(compareFn) {
      ...
      for (var arr = this.__array, len = arr.length, i = len - 1; i > -1; i--) {
        if (arr[i] !== undefined) {
          if (i < len - 1)
            arr.splice(i + 1);
          break;
      ...
      for (var arr = this.__array, len = arr.length, i = 0; i < len; i++) {
        if (arr[i] !== undefined) {
          if (i > 0)
            arr.splice(0, i);
          break;
