    ...
    writeCompileVersion: async function() {
      return mkParentPath(this.getOutputDir())
        .then(() => {
          ...
          let s = "compiler version: " + qx.tool.compiler.Version.VERSION + "\n";
          s += "library versions:\n";
          let libs = this.getAnalyser().getLibraries();
          libs.forEach(lib => {
             s += lib.getNamespace() + ": "  + lib.getVersion() + "\n";
          });
          writeFile(path.join(this.getOutputDir(), "buildinfo.txt"), s, { encoding: "utf8" })
        });
    },
