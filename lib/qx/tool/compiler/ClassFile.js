...
qx.Class.define("qx.tool.compiler.ClassFile", {
  ...
  members: {
    ...
    load: function (callback) {
      ...
      fs.readFile(this.getSourcePath(), {encoding: "utf-8"}, function (err, src) {
        ...
        util.mkParentPath(outputPath, function(err) {
          if (err)
            return callback(err);
          fs.writeFile(outputPath, result.code + "\n\n//# sourceMappingURL=" + t.__meta.name + ".js.map", {encoding: "utf-8"}, function(err) {
            if (err)
              return callback(err);
    ...
    _compileDbClassInfo: function() {
      ...
      if (meta.aliases) {
        dbClassInfo.aliases = {};
        for (var name in meta.aliases.aliasMap)
          dbClassInfo.aliases[name] = meta.aliases.aliasMap[name];
      }
      if (meta.themeMeta) {
        dbClassInfo.themeMeta = {};
        for (var name in meta.themeMeta.themeMetaMap)
    ...
    _babelClassPlugin: function() {
      ...
      const CODE_ELIMINATION_VISITOR = {
        CallExpression(path) {
          ...
          if ((name === "qx.core.Environment.select" || name === "qx.core.Environment.get") &&
              types.isLiteral(path.node.arguments[0])) {
            const arg = path.node.arguments[0];
            const env = Object.assign({}, qx.tool.compiler.ClassFile.ENVIRONMENT_CONSTANTS,
              t.__analyser.getEnvironment());
            if (arg.value.match(/module/))
              debugger;
            ...
            if (envValue !== undefined) {
              if (name === "qx.core.Environment.get") {
                ...
                if (!option) {  // try to find default value
            ...
            t._requireClass("qx.core.Environment", { location: path.node.loc, defer: true });
          }
        },
      ...
      function collectJson(node) {
        ...
        if (node.type == "ObjectExpression") {
          ...
          if (node.operator == "-") {
            var tmp = collectJson(node.argument);
        ...
        Property(path) {
          ...
          if (keyName == "extend") {
            ...
            if (!prop.value.properties) {
              ...
              var meta = makeMeta("themeMeta", null, prop);
              meta.themeMetaMap = {};
              ...
              if (TYPE[name]) {
                ...
                if (dbClassInfo.dependsOn) {
                  ...
                  Object.keys(dbClassInfo.dependsOn).forEach(key => {
                    var tmp = copyInfo.dependsOn[key] = dbClassInfo.dependsOn[key];
                    if (tmp.load) {
                      delete tmp.load;
                      tmp.require = true;
                      hasLoadDeps = true;
