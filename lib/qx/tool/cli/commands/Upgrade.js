  ...
  members: {
    ...
    process: async function() {
      ...
      try {
        ...
        throw new qx.tool.cli.Utils.UserError("Cannot read config.json: " + (ex.message||ex.stack));
      }
      try {
        configJson = parseJson(data);
      } catch (ex) {
        throw new qx.tool.cli.Utils.UserError("Cannot parse config.json: " + (ex.message||ex.stack));
      }
      
      t.debug(">>> Loading compile.json (if there is one)");
      try {
        data = await readFile("compile.json", {encoding: "utf8"});
        try {
          compileAst = JsonToAst.parseToAst(data, { verbose: true });
          compileJson = JsonToAst.astToObject(compileAst);
        } catch (ex) {
          throw new qx.tool.cli.Utils.UserError("Cannot parse compile.json: " + (ex.message||ex.stack));
        }
      } catch (err) {
        if (err.code !== "ENOENT") {
          throw err; 
      ...
      if (compileJson.libraries) {
        compileJson.libraries = compileJson.libraries.map(function(path) {
          if (typeof path === "object" && typeof path.path === "string") {
            ...
            throw new qx.tool.cli.Utils.UserError("Cannot interpret configuration for libraries");
          }
          return path;
        });
      }
      ...
      if (!libLookup["qx"]) {
        ...
        if (!dir) {
          throw new qx.tool.cli.Utils.UserError("Cannot find Qooxdoo in " + dir);
        } else {
      ...
      str = JsonToAst.reprint(compileJson, compileAst);
