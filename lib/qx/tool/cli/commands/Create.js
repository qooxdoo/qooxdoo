    ...
    process: async function() {
      ...
      try {
        ...
        if ( argv.qxpath !== undefined ){
          throw new qx.tool.cli.Utils.UserError(`Invalid --qxpath value. Cannot find qooxdoo framework folder at ${argv.qxpath}`);
        }          
        if (argv.noninteractive){
          throw new qx.tool.cli.Utils.UserError("Cannot find qooxdoo framework folder. Please provide the path using the --qxpath option.")
        }
      ...
      for(let var_name in template_vars){
        ...
        if (argv.noninteractive){
          ...
          throw new qx.tool.cli.Utils.UserError(`Cannot skip required value for '${var_name}'.`);
        }
        // ask user
      ...
      if( ! fs.existsSync(appdir) ){
        ...
        if (! fs.existsSync(parentDir) ) throw new qx.tool.cli.Utils.UserError(`Invalid directory ${appdir}`);
        try{
          fs.accessSync(parentDir,fs.constants.W_OK);
        } catch(e){
          throw new qx.tool.cli.Utils.UserError(`Directory ${parentDir} is not writable.`);
        }
        fs.mkdirSync(appdir);
      }
      ...
      if ( argv.type && ! fs.existsSync( skeleton_dir ) ) {
        throw new qx.tool.cli.Utils.UserError(`Application type <${answer}> does not exist or has not been implemented yet.`);
      }
          ...
          if (stats.isFile() ) {
            ...
            if ( sourceFile.includes(".tmpl") ) {
              ...
              if( fs.existsSync(targetFile) ){
                throw new qx.tool.cli.Utils.UserError(`${targetFile} already exists.`); // todo: handle overwriting
              }
