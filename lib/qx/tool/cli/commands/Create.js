  ...
  members: {
    ...
    process: async function() {
      ...
      try {
        ...
        throw new qx.tool.cli.Utils.UserError("Cannot find qooxdoo framework folder.")
      }
      ...
      for(let var_name of Object.getOwnPropertyNames(template_vars)){
        ...
        switch(var_name){
          ...
          if (!value.match(/^([a-zA-Z_$][0-9a-zA-Z_$]*\.?)+$/)) { 
            throw new qx.tool.cli.Utils.UserError(`Illegal characters in namespace "${value}."`); 
          } 
          break; 
      ...
      if ( argv.type && ! fs.existsSync( skeleton_dir ) ) {
        throw new qx.tool.cli.Utils.UserError(`Application type '${argv.type}' does not exist or has not been implemented yet.`);
      }
        ...
        for (let part of files) {
          ...
          if (stats.isFile() ) {
            ...
            if ( sourceFile.includes(".tmpl") ) {
              ...
              if (argv.verbose) console.info(`>>> Creating ${targetFile} from template ${sourceFile}...`);
