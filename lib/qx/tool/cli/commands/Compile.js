...
qx.Class.define("qx.tool.cli.commands.Compile", {
  ...
  members: {
     ...
     */
    configure: async function(filename) {
      var t = this;
      return t.loadConfig(filename).then((data) => {
    ...
    createMakerFromConfig: async function(data) {
      ...
      if (!data.libraries.length && await fs.existsAsync("Manifest.json"))
        maker.addLibrary(".");
      ...
      if (!qxLib) {
        ...
        if (await fs.existsAsync("node_modules/qooxdoo/Manifest.json")) {
          await maker.addLibrary("node_modules/qooxdoo");
          return maker;
        }
