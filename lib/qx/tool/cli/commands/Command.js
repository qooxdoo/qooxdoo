...
qx.Class.define("qx.tool.cli.commands.Command", {
  ...
  statics:{
    ...
    /**
     * Migrate files/schemas
     * @param {[]} migrate_files Array containing arrays of [new name, old name]
     * @private
     */
    migrate(migrate_files) {
      let needMigration = [];
      for (let [newPath, oldPath] of migrate_files) {
        if (!fs.existsSync(newPath) && fs.existsSync(oldPath)) {
          needMigration.push([newPath, oldPath]);
        }
      }
      if (needMigration.length) {
        let migrate = process.argv.includes("--migrate");
        let quiet = process.argv.includes("--quiet") || process.argv.includes("-q");
        if (migrate) {
          ...
          if (!quiet) {
            ...
            console.warn("*** You might have to adapt your VCS settings (such as .gitignore files).");
          }
          process.exit(0);
        } else {
          console.warn(`*** Warning: Some metadata filenames have changed.`);
          console.warn(`*** Please run 'qx package --migrate' flag to rename:`);
          for (let [newPath, oldPath] of needMigration) {
