...
qx.Class.define("qx.tool.cli.commands.Command", {
  ...
  members: {
    ...
    async process() {
      ...
      this.argv.announce = true;
      await (new qx.tool.cli.commands.package.Migrate(this.argv)).process();
    },
    ...
    getGlobalQxPath: async function() {
      ...
      if (await fs.existsAsync("node_modules/@qooxdoo/framework/framework/Manifest.json")) {
        return path.resolve("node_modules/@qooxdoo/framework/framework");
      }
     ...
     * @return []
     */
    checkFilesToMigrate(migrateFiles) {
      let needMigration = [];
      for (let [newPath, oldPath] of migrateFiles) {
        if (!fs.existsSync(newPath) && fs.existsSync(oldPath)) {
          needMigration.push([newPath, oldPath]);
        }
      }
    ...
    /**
     * Migrate files/schemas or announces the migration. Exits the current process in both cases.
     * @param {[]} migrateFiles Array containing arrays of [new name, old name]
     ...
     * @private
     */
    async migrate(migrateFiles, replaceInFiles, annouceOnly=false) {
      let quiet = this.argv.quiet;
      let needMigration = this.checkFilesToMigrate(migrateFiles);
      if (annouceOnly) {
        // announce migration
        if (needMigration.length || replaceInFiles) {
          console.warn(`*** Warning: Some metadata filenames have changed. The following files will be renamed:`);
          for (let [newPath, oldPath] of needMigration) {
        ...
        console.warn(`*** Please run 'qx package migrate' to apply the changes. If you don't want this, downgrade to a previous version of the compiler.`);
        process.exit(1);
      } else {
        // apply migration
        for (let [newPath, oldPath] of needMigration) {
        ...
        if (!quiet) {
          console.warn("*** Now run 'qx package upgrade --reinstall' to fix path names in the lockfile.");
        }
        process.exit(0);
      }
    }
