  ...
  members: {
    ...
    process: async function() {
      ...
      async function updateFromGitHubAPI() {
        ...
        for (let repo of result.data) {
          ...
          for (let release of releases) {
            ...
            for (let [index, {path:manifest_path}] of manifests.entries()) {
              ...
              // @deprecated
              var qx_version_range = (data.requires && data.requires["@qooxdoo/framework"]) || data.info["qooxdoo-range"];
              ...
              num_libraries++;
              if (argv.verbose) {
                console.log(`>>> ${name} ${tag_name}: Found '${data.info.name}' contrib (compatible with ${qx_versions})`);
              } else if (!argv.quiet) {
        ...
        // wrap-up
        self.getCache().version = qx.tool.cli.ConfigSchemas.lockfile.version;
