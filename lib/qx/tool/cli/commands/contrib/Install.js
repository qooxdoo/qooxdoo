...
qx.Class.define("qx.tool.cli.commands.contrib.Install", {
  ...
  statics: {
    getYargsCommand: function() {
      return {
        ...
        examples: 
           * qx contrib install name: Install latest version
           * qx contrib install name@v0.0.2: Install version 0.0.2,
           * qx contrib install name@master: Install version from github`,
        builder: {
          "release" : {
            alias: "r",
          ...
          "from-path": {
            alias: "p",
            nargs: 1,
            describe: "Install a library/the given contrib library from a local path"
          }
        },
  ...
  members: {
    ...
    process: async function() {
      ...
      this.argv.uri = this.argv.uri || this.argv["contrib_uri@release_tag"];
    ...
    async __updateInstalledLibraryData(uri, id, download_path, writeToManifest) {
      let {repo_name, contrib_path} = uri ? this.__getUriInfo(uri) : {repo_name:"", contrib_path:""};
    ...
    __installApplication: async function(downloadPath) {
      ...
      let compileJson = await qx.tool.compiler.utils.Json.loadJsonAsync(qx.tool.cli.ConfigSchemas.compiler.filename);
      ...
      await qx.tool.compiler.utils.Json.saveJsonAsync(qx.tool.cli.ConfigSchemas.compiler.filename, compileJson);
      if (this.argv.verbose) {
