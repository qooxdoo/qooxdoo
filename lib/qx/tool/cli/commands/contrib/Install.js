...
qx.Class.define("qx.tool.cli.commands.contrib.Install", {
  ...
  members: {
    ...
    process: async function() {
      ...
      if (!argv["contrib_uri@release_tag"]) {
        await this.__loadFromContribJson();
        return;
      }

      if (!argv.release) {
        let s = argv["contrib_uri@release_tag"].split(/@/);
        argv["contrib_uri@release_tag"] = s[0];
        argv.release = s[1];
      }
      ...
      // install repo
      await this.__installRepo(argv["contrib_uri@release_tag"], argv.release, true);
      // save data
    ...
    __installRepo: async function(repoName, tagName, writeToManifest) {
      ...
      for( let { info, path:filename } of release_data.manifests ) {
        ...
        let index = this.__data.libraries.findIndex((elem)=>{
          if (elem.uri) {
            return elem.uri === current_uri;
          }
        });
        library_elem = {
        ...
        if (index >= 0) {
          ...
          if (! argv.quiet) console.info(`>>> Updating already existing config.json entry for'${current_uri}'.`);
        } else {
      ...
      if (!found) {
        throw new qx.tool.cli.Utils.UserError(`The contrib library identified by '${repoName}' could not be found.`);
      }
    },
