...
qx.Class.define("qx.tool.cli.commands.contrib.Publish", {
  ...
  members: {
    ...
    process: async function() {
      ...
      let manifest = qx.tool.compiler.utils.Json.parseJson( fs.readFileSync(manifest_path,"utf-8") );
      ...
      let result, topics;
      try {
        result = await octokit.repos.listTopics({owner, repo});
      ...
      if (manifest.info["qooxdoo-range"]) {
        if( ! argv.quiet) console.info(`Deprecated key "manifest.info.qooxdoo-range" found, will be changed to manifest.requires.qooxdoo-sdk`);
        semver_range = manifest.info["qooxdoo-range"];
        delete manifest.info["qooxdoo-range"];
      }
      semver_range = manifest.requires["qooxdoo-sdk"] || semver_range;
