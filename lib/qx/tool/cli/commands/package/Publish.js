    ...
    process: async function() {
      ...
      if (fs.existsSync(index_path)) {
        ...
        if (!argv.force && manifest.provides.namespace.includes(".demo")) {
          throw new qx.tool.cli.Utils.UserError("This seems to be the library demo. Please go into the library directory to publish the library.");
        }
        libraries = [{ path: "."}];
      }
      ...
      for (let library of libraries) {
        ...
        if (manifest.info["qooxdoo-range"]) {
          if (!argv.quiet) {
            console.info(`Deprecated key "manifest.info.qooxdoo-range" found, will be changed to manifest.requires.qooxdoo-sdk`);
          }
          semver_range = manifest.info["qooxdoo-range"];
          delete manifest.info["qooxdoo-range"];
          needWrite = true;
        }
        semver_range = (manifest.requires && manifest.requires["qooxdoo-sdk"]) || semver_range;
        ...
        if (needWrite) {
          ...
          manifest.requires["qooxdoo-sdk"] = semver_range;
        }
