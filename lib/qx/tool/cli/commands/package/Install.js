...
qx.Class.define("qx.tool.cli.commands.package.Install", {
  ...
  members: {
    ...
    async installFromLocaPath(local_path, library_uri) {
      ...
      if (this.argv.verbose) {
        console.info(`>>> To be installed: ${library_uri || "local libarary"} from ${local_path}`);
      }
    ...
    process: async function() {
      ...
      if (this.argv.fromPath) {
        ...
        if (id) {
          throw new qx.tool.cli.Utils.UserError(`Version identifier cannot be used when installing from local path.`);
        }
    ...
    async __readConfigData() {
      this.__manifest = await qx.tool.compiler.utils.Json.loadJsonAsync(qx.tool.cli.ConfigSchemas.library.filename);
      if (!this.__manifest.requires) {
        this.__manifest.requires = {};
      }
      this.__data = await this.getPackageData();
    },
    ...
    __getUriInfo(uri) {
      if (!uri) {
        throw new qx.tool.cli.Utils.UserError("No package resource identifier given");
      }
      ...
      if (!this.getCache().repos.data[repo_name]) {
        throw new qx.tool.cli.Utils.UserError(`A repository '${repo_name}' cannot be found.`);
      }
      return {
        repo_name,
        package_path
    ...
    __installFromRelease: async function(uri, tag_name, writeToManifest) {
      ...
      if (!found) {
        throw new qx.tool.cli.Utils.UserError(`The package/library identified by '${uri}' could not be found.`);
      }
    },
    ...
    async __installFromPath(uri, dir, writeToManifest=false) {
      let qooxdoo_version = await this.getUserQxVersion();
      if (this.argv.verbose) {
        console.info(`>>> Installing '${uri}' from '${dir}' for qooxdoo version ${qooxdoo_version}`);
      }
    ...
    async __updateInstalledLibraryData(uri, id, download_path, writeToManifest) {
      ...
      let manifest_path = path.join(library_path, qx.tool.cli.ConfigSchemas.library.filename);
      if (!fs.existsSync(manifest_path)) {
        throw new qx.tool.cli.Utils.UserError(`No manifest file in '${download_path}'.`);
      }
        ...
        switch (lib_uri) {
          ...
          case "@qooxdoo/framework": {
            ...
            if (!semver.satisfies(qxVer, lib_version, {loose: true}) && this.argv.ignore) {
              throw new qx.tool.cli.Utils.UserError(
                `Library '${lib_uri}' needs @qooxdoo/framework version ${lib_version}, found ${qxVer}`
              );
            }
            break;
          }
    ...
    __installApplication: async function(downloadPath) {
      let manifest = await qx.tool.compiler.utils.Json.loadJsonAsync(path.join(downloadPath, qx.tool.cli.ConfigSchemas.library.filename));
      if (!manifest.provides || !manifest.provides.application) {
        return false;
      }
      // FIXME: this won't use compile.js
      let compileData = await qx.tool.compiler.utils.Json.loadJsonAsync(qx.tool.cli.ConfigSchemas.compiler.filename);
      ...
      await qx.tool.compiler.utils.Json.saveJsonAsync(qx.tool.cli.ConfigSchemas.compiler.filename, compileData);
      if (this.argv.verbose) {
    ...
    __downloadLibrariesInLockfile: async function() {
      if (this.argv.verbose) {
        console.info(`>>> Downloading libraries listed in ${qx.tool.cli.ConfigSchemas.lockfile.filename}...`);
      }
      let libraries = (await this.getPackageData()).libraries;
    ...
    __download: async function(repo_name, treeish=null, force = false) {
      ...
      let parts = [process.cwd(), this.self(arguments).cache_dir, dir_name];
      let dir_exists;
      ...
      if (!force && dir_exists) {
        if (this.argv.verbose) {
          console.info(`>>> Repository '${repo_name}', '${treeish}' has already been downloaded to ${download_path}. To download again, execute 'qx clean'.`);
        }
      } else {
        if (this.argv.verbose) {
          console.info(`>>> Downloading repository '${repo_name}', '${treeish}' from ${url} to ${download_path}`);
        }
        try {
