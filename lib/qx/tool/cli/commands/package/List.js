...
qx.Class.define("qx.tool.cli.commands.package.List", {
  ...
  members: {
    ...
    process: async function() {
      ...
      if (repos_cache.list.length === 0 || this.getCache().version !== qx.tool.cli.ConfigSchemas.lockfile.version) {
    ...
    __createIndexes : async function(qooxdoo_version) {
      if (this.argv.installed) {
        ...
        let libData = await this.getPackageData();
        for (let lib of libData.libraries) {
          if (!lib.repo_name) {
            let manifest_path = path.join(process.cwd(), lib.path, qx.tool.cli.ConfigSchemas.library.filename);
            let manifest = await qx.tool.compiler.utils.Json.loadJsonAsync(manifest_path);
