...
qx.Class.define("qx.tool.cli.commands.Package", {
  ...
  members: {
     ...
     */
    __migrate() {
      let self = this.self(arguments);
      let migrate_files = [
        [this.getLockfilePath(), path.join(process.cwd(), qx.tool.cli.ConfigSchemas.lockfile.legacy_filename)],
        [self.cache_dir, self.legacy_cache_dir],
        [self.package_cache_name, self.legacy_package_cache_name]
      ];
      let needMigration = [];
      for (let [newName, oldName] of migrate_files.entries()) {
        if (!fs.existsSync(newName) && fs.existsSync(oldName)) {
          try {
            fs.renameSync(oldName, newName);
            needMigration.push([newName, oldName]);
          } catch (e) {
            console.error(`Renaming '${oldName}' to '${newName}' failed: ${e.message}.`);
            process.exit(1);
          }
        }
      }
      if (needMigration.length && !this.argv.quiet) {
        console.warn(`*** Warning; Some metadata filenames have changed ***`);
        for (let [newName, oldName] of needMigration.entries()) {
          console.warn(`Renamed '${oldName}' to '${newName}'.`);
        }
        console.warn(`You might have to adapt your VCS settings and commit the changes.`);
      }
    },
