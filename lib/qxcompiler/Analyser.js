...
module.exports = qx.Class.define("qxcompiler.Analyser", {
  ...
  members: {
    ...
    open: function(cb) {
      ...
      Promise.all(
        ...
        .then(() => {
          ...
          util.promisifyThis(t.initialScan, t);
        })
        .then(() => {
          log.debug("Saving database");
          util.promisifyThis(t.saveDatabase, t);
        })
        .then(cb);
    },
    ...
    analyseClasses: function (cb) {
      ...
      function updateMetaData(classname, meta) {
      ...
      async function loadMetaData(classname, meta) {
        var filename = qxcompiler.ClassFile.getOutputPath(t, classname) + "on";
      ...
      function analyzeMeta() {
        ...
        for (var classname in compiledClasses) {
          ...
          var meta = data.classFile.getMeta();
          updateMetaData(classname, meta);
          calcDescendants(classname, meta);
          promises.push(saveMetaData(classname, meta));
        }
          ...
          if (!compiledClasses[classname] && db.classInfo[classname]) {
            promises.push(
                loadMetaData(classname)
                  .then((meta) => {
                    calcDescendants(classname, meta);
                    return saveMetaData(classname, meta);
                  })
              );
          }
        }
    ...
    getCldr: async function(locale) {
      ...
      if (cldr)
        return cldr;
      return qxcompiler.Cldr.loadCLDR(t.getQooxdooPath(), locale)
        .then((cldr) => {
