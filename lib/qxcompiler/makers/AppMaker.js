                ...
                async.each(t.__applications,
                    function(application, cb) {
                      ...
                      if (application.getFatalCompileErrors()) {
                        qxcompiler.Console.print("qxcompiler.maker.appFatalError", application.getName());
                        cb();
                      } else {
                        qxcompiler.Console.print("qxcompiler.maker.writingApplication", target, application.getName());
                        var env = {
                          "qx.compilerVersion": qxcompiler.Version.VERSION
                        };
                        env = Object.assign(env, qxcompiler.makers.AbstractAppMaker.DEFAULT_ENVIRONMENT, t.getEnvironment());
                        target.generateApplication(application, env, cb);
                      }
                    },
                    cb);
              }
