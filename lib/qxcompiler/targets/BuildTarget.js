...
module.exports = qx.Class.define("qxcompiler.targets.BuildTarget", {
  ...
  members: {
    ...
    _writeApplication: function(cb) {
      ...
      // Do these via eachSeries (not via each() because that's parallel) so that we avoid
      //  issues where two event loops are writing to the same path because this can cause
      //  confusing conflicts where (eg) a test for whether something exists returns false
      //  but then the file/directory unexpectedly appears.
      async.eachSeries(libraries, function (library, cb) {
