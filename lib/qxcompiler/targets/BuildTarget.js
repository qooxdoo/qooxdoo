    ...
    _afterWriteApplication: function(compileInfo, cb) {
      ...
      Object.keys(compileInfo.configdata.loader.packages).forEach(pkgId => {
        ...
        p = p
          ...
          .then(() => qxcompiler.files.Utils.safeUnlink(partJs + ".unminified"))
          .then(() => qxcompiler.files.Utils.safeRename(partJs, partJs + ".unminified"))
          
          .then(() => fs.readFileAsync(partJs + ".map", "utf8").then(data => partSourceMap = data))
          .then(() => qxcompiler.files.Utils.safeUnlink(partJs + ".unminified.map"))
          .then(() => qxcompiler.files.Utils.safeRename(partJs + ".map", partJs + ".unminified.map"))
          ...
          .then(() => {
            ...
            if (err) {
              if (err.name == "SyntaxError") {
                qxcompiler.Console.print("qxcompiler.build.uglifyParseError", err.line, err.col, err.message, "part-" + pkgId + ".js");
              }
