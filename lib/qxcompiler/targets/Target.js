...
module.exports = qx.Class.define("qxcompiler.targets.Target", {
  ...
  members: {
     ...
     */
    _getApplicationRoot: function(application) {
    ...
    _getOutputRootUri: function(application) {
      ...
      if (!targetUri) {
        var dir = this._getApplicationRoot(application);
    ...
    _writeIndexHtml: async function(compileInfo) {
      ...
      var ns = classname.substring(0, pos);
      var library = this.getAnalyser().findLibrary(ns);
      var bootDir = path.join(library.getRootDir(), library.getBootPath());
      var stats = await stat(bootDir);
