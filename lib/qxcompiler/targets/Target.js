  ...
  members: {
      ...
      util.mkpath(appRootDir + "script", function(err) {
        ...
        async.parallel([
                              ...
                              function(localeId, cb) {
                                analyser.getTranslation(library, localeId, function(err, translation) {
                                  ...
                                  dbClassInfo.translations.forEach(function(transInfo) {
                                    var entry = translation.getEntry(transInfo.msgid);
                                    if (entry) {
          ...
          function(err) {
            if (err)
              return cb && cb(err);
            t._writeApplication(foundAssets, cb);
          });
      });
    },
    ...
    _writeApplication: function(assets, cb) {
      ...
      async.parallel([
          ...
          function (cb) {
            t._writeIndexHtml(function(err) {
              cb(err);
            });
          },
     ...
     * @private
     */
    _afterWriteApplication: function(cb) {
      cb()
    }
  }
});
