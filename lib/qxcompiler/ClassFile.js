...
qx.Class.define("qxcompiler.ClassFile", {
  ...
  members: {
      ...
      fs.readFile(this.getSourcePath(), {encoding: "utf-8"}, function (err, src) {
        ...
        util.mkParentPath(outputPath, function(err) {
          ...
          fs.writeFile(outputPath, result.code, {encoding: "utf-8"}, function(err) {
            if (err)
              return callback(err);
            fs.writeFile(outputPath + "on", JSON.stringify(t.__meta, null, 2), {encoding: "utf-8"}, function(err) {
    ...
    _compileDbClassInfo: function() {
      ...
      // Class heirararchy
      dbClassInfo.extends = meta.extend;
      dbClassInfo.include = meta.include.slice(0);
      dbClassInfo.implement = meta.implement.slice(0);
    ...
    _babelClassPlugin: function() {
      ...
      function getJsDoc(comment) {
        if (qx.lang.Type.isArray(comment))
          comment = comment[comment.length - 1]||null;
        if (!comment)
          return null;
        var result = qxcompiler.jsdoc.Parser.parseComment(comment.value);
        result.forEach((elem) => {
      ...
      function makeMeta(sectionName, functionName, node) {
        ...
        if (node.leadingComments) {
          ...
          if (jsdoc) {
            if (jsdoc.ignore) {
              jsdoc.ignore.forEach(function(name) {
                t.addIgnore(name);
              });
            }
            meta.jsdoc = jsdoc;
      ...
      var CLASS_DEF_VISITOR = {
        Property(path) {
          ...
          if (keyName == "extend") {
            t.__meta.extend = collapseMemberExpression(prop.value);
            t._requireClass(t.__meta.extend, { location: path.node.loc });

          } else if (keyName == "implement") {
            path.skip();
            path.traverse(COLLECT_CLASS_NAMES_VISITOR, { collectedClasses: t.__meta.implement });

          } else if (keyName == "include") {
            path.skip();
            path.traverse(COLLECT_CLASS_NAMES_VISITOR, { collectedClasses: t.__meta.include });
            ...
            path.traverse({
              Property(path) {
                ...
                makeMeta("statics", t.__functionName, path.node);
                path.traverse(VISITOR);
            ...
            path.traverse({
              Property(path) {
                ...
                t.__functionName = getKeyName(path.node.key);
                makeMeta("members", t.__functionName, path.node);
                path.traverse(VISITOR);
                t.__functionName = null;
              },
              FunctionDeclaration: functionDeclOrExpr,
            ...
            prop.value.properties.forEach(function(pdNode) {
              ...
              meta.property = collectJson(pdNode.value);
            });
              ...
              if (jsdoc) {
                if (jsdoc["@use"]) {
                  jsdoc["@use"].forEach(function(elem) {
                    t._requireClass(elem.body, {where: "use", load: false, location: path.node.loc });
                  });
                }
                if (jsdoc["@require"]) {
                  jsdoc["@require"].forEach(function(elem) {
                    t._requireClass(elem.body, {where: "require", load: false, location: path.node.loc });
                  });
                }
                if (jsdoc["@optional"]) {
              ...
              if (name == "qx.core.Environment.add") {
                ...
                if (t.__definingType == "Mixin") {
                  ...
                  if (t.__functionName == "$$constructor")
                    expr = expandMemberExpression(t.__meta.extend + ".constructor.call");
                  else
                ...
                if (t.__definingType == "Mixin") {
                  ...
                  if (t.__functionName == "$$constructor")
                    expr = expandMemberExpression(t.__meta.extend + ".constructor." + methodName);
                  else
          ...
          exit: function(path) {
            if (types.isMemberExpression(path.node.callee)) {
              ...
              if (name == "qx.Class.define" || name == "qx.Mixin.define" || name == "qx.Theme.define"
                ...
                if (index === -1)
                  throw new Error("Cannot find node in CallExpression.exit for " + t.__className);
