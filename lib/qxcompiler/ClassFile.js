  ...
  members: {
    ...
    _babelClassPlugin: function() {
      ...
      var functionDeclOrExpr = {
        enter(path) {
          var node = path.node;
          console.log("Function enter: " + (node.id ? node.id.name : "anonymous"));
          if (node.id)
        ...
        exit(path) {
          var node = path.node;
          console.log("Function exit: " + (node.id ? node.id.name : "anonymous"));
          t.popScope();
        }
      };
    ...
    _requireClass: function (name, opts, cb) {
      ...
      if (name == "qx.bom.element.Style" && (requireOpts.load || (opts && opts.load)))
        debugger;
      if (opts)
        for (var key in opts)
