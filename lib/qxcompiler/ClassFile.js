...
qx.Class.define("qxcompiler.ClassFile", {
              ...
              if (hints) {
                if (hints.use) {
                  hints.use.forEach(function(name) {
                    t._requireClass(name, {where: "use", load: false});
                  });
                }
                if (hints.require) {
                  hints.require.forEach(function(name) {
                    t._requireClass(name, {where: "require", load: false});
                  });
                }
                if (hints.optional) {
    ...
    addEnvCheck: function(name) {
      ...
      if (qxcompiler.ClassFile.ENVIRONMENT_CONSTANTS[name] === undefined) {
        ...
        if (entry && entry.className) {
          t._requireClass(entry.className, { load: requiredOpts.load });
          dest.className = entry.className;
        } else if (!entry) {
          t._requireClass(name, { load: requiredOpts.load });
        }
      }
    },
  ...
  statics: {
    ...
    MARKERS: {
      ...
      "defer.unsafe": "Unsafe use of 'defer' method to access external class"
    },
