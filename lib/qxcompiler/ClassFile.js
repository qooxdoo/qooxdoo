...
qx.Class.define("qxcompiler.ClassFile", {
  ...
  members: {
        ...
        comments.forEach(function(comment) {
          ...
          for (var j = 0; j < lines.length; j++) {
            ...
            if (match) {
              ...
              if (!result[key])
                result[key] = [];
              result[key].push(match[2].replace(/#.*/, ""));
            }
          }
        });
        return result;
      ...
      var CLASS_DEF_VISITOR = {
        Property(path) {
          ...
          if (keyName == "extend") {
            ...
            t._requireClass(t.__extendClassName, { location: path.node.loc });
            ...
            path.traverse({
              Property(path) {
                ...
                if (path.node.leadingComments) {
                  if (path.node.leadingComments) {
                    var hints = getHints(path.node.leadingComments);
                    if (hints) {
                      if (hints.ignore) {
                        hints.ignore.forEach(function(name) {
                          t.addIgnore(name);
                        });
                      }
                    }
        ...
        Program(path) {
          ...
          node.body.forEach(function (node) {
            if (node.leadingComments) {
              var hints = getHints(node.leadingComments);
              if (hints) {
                if (hints.use) {
                  hints.use.forEach(function(name) {
                    t._requireClass(name, {where: "use", load: false, location: path.node.loc });
                  });
                }
                if (hints.require) {
                  hints.require.forEach(function(name) {
                    t._requireClass(name, {where: "require", load: false, location: path.node.loc });
                  });
                }
                if (hints.optional) {
                  hints.optional.forEach(function(name) {
                    t.addIgnore(name);
                  });
                }
                if (hints.ignore) {
                ...
                if (hints.asset) {
                  hints.asset.forEach(function(name) {
                    t._requireAsset(name);
                  });
                }
              }
            }
            ...
            if (types.isMemberExpression(path.node.callee)) {
              ...
              if (name == "qx.core.Environment.add") {
                ...
                if (t.__definingType == "Mixin") {
                  ...
                  if (t.__functionName == "$$constructor")
                    expr = expandMemberExpression(t.__extendClassName + ".constructor." + methodName);
                  else
    ...
    addMarker: function(msgId, pos) {
      ...
      if (args.length)
        marker.args = args;
      if (msgId == "symbol.unresolved" && args[0] === "_wrapper.$$original")
        debugger;
  ...
  statics: {
    ...
    getSourcePath: function(library, className) {
      return library.getRootDir() + pathModule.sep + library.getSourcePath() + pathModule.sep + className.replace(/\./g, pathModule.sep) + ".js";
    },
    ...
    getOutputPath: function(analyser, className) {
      var path = analyser.getOutputDir() + "transpiled" + pathModule.sep + className.replace(/\./g, pathModule.sep) + ".js";
      return path;
    },
