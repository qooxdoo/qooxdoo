...
qx.Class.define("qxcompiler.app.Application", {
  ...
  members: {
    ...
    calcDependencies: function () {
      var t = this;
      var Console = qxcompiler.Console.getInstance();
      ...
      if (this.__parts && this.__parts.length) {
        ...
        t.__parts.forEach((part) => {
          if (partsByName[part.getName()])
            throw new Error(Console.decode("qxcompiler.application.duplicatePartNames", part.getName()));
          var partData = {
              name: part.getName(),
        ...
        if (!bootPart)
          throw new Error(Console.decode("qxcompiler.application.noBootPart")); 
      } else {
        bootPart = {
            name: "boot",
            include:[ "*" ],
        ...
        parts.forEach((part) => {
          ...
          if (result !== null) {
            ...
            // Exact
              ...
              if (result === "exact")
                Console.print("qxcompiler.application.conflictingExactPart", classname, part.name, classData.best.name);
            ...
            // Wildcard
              ...
              if (result === "exact") {
                ...
                if (lastMatch === result)
                  Console.print("qxcompiler.application.conflictingBestPart", classname, part.name, classData.best.name);
      ...
      var needed = new qxcompiler.utils.IndexedArray();
      var neededIndex = 0;
      ...
      if (parts && parts.length > 1) {
        ...
        parts.forEach((part) => {
          ...
          if (part.dependsOn.some(check))
            Console.print("qxcompiler.application.partRecursive", part.name);
        });
      }
     ...
     * @param resManager {qxcompiler.resources.Manager} the resource manager
    ...
    _applyType: function(value, oldValue) {
      var loader = path.join(__dirname, "loader-" + value + ".tmpl.js");
