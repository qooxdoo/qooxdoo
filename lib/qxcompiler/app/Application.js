      ...
      if (this.__parts && this.__parts.length) {
        ...
        t.__parts.forEach((part) => {
          if (partsByName[part.getName()])
            throw new Error(Console.decode("application.duplicatePartNames", part.getName()));
          var partData = {
              name: part.getName(),
        ...
        if (!bootPart)
          throw new Error(Console.decode("application.noBootPart")); 
      } else {
        bootPart = {
            name: "boot",
            include:[ "*" ],
        ...
        parts.forEach((part) => {
          ...
          if (result !== null) {
            ...
            // Exact
              ...
              if (result === "exact")
                Console.print("application.conflictingExactPart", classname, part.name, classData.best.name);
            ...
            // Wildcard
              ...
              if (result === "exact") {
                ...
                if (lastMatch === result)
                  Console.print("application.conflictingBestPart", classname, part.name, classData.best.name);
      ...
      if (parts && parts.length > 1) {
        ...
        parts.forEach((part) => {
          ...
          if (part.dependsOn.some(check))
            Console.print("application.partRecursive", part.name);
        });
      }
