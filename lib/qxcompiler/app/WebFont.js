...
qx.Class.define("qxcompiler.app.WebFont", {
  ...
  members: {
    ...
    getBootstrapCode : function() {
      //TODO: execute this code on Application startup
      return "console.log('Look mum - no hands!');";
    },
    ...
    generateForTarget: async function(target) {
      this.getResources().some(function(resource) {
        ...
        if (resource.match(/^https?:\/\//)) {
          this._loadRemoteFont(resource).then((data) => {
            //TODO: This needs to go to the applications qx.$$resources
            console.log(data);
            Promise.resolve();
          }).catch((err) => {
            console.log("ERROR");
