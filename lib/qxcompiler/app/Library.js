 ...
 * the library itself
 */
qx.Class.define("qxcompiler.app.Library", {
  ...
  properties: {
    ...
    /** The path to boot files, relative to rootDir */
    bootPath: {
      init: "source/boot",
      check: "String"
    },
  ...
  members: {
    ...
    loadManifest: function(rootDir, cb) {
      ...
      fs.readFile(rootDir + "/Manifest.json", { encoding: "utf8" }, function(err, data) {
        ...
        if (data.provides.boot)
          t.setBootPath(data.provides.boot);
        if (data.provides.webfonts) {
          var fonts = [];
          data.provides.webfonts.forEach((wf) => {
            var font = new qxcompiler.app.WebFont(t).set(wf);
            fonts.push(font);
          });
          t.setWebFonts(fonts);
        }
                ...
                fs.stat(path.join(folder, filename), function(err, stat) {
                  ...
                  if (stat.isDirectory()) {
                    ...
                    if (tmp.length)
                      tmp += ".";
                    tmp += filename;
                    return scanDir(path.join(folder, filename), tmp, cb);
                  }
...
module.exports = qxcompiler.app.Library;
