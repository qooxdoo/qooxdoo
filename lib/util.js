  ...
  async.eachSeries(
      ...
      function(seg, cb) {
        ...
        fs.exists(made, function(exists) {
          if (!exists)
            return fs.mkdir(made, cb);
          fs.stat(made, function(err, stat) {
            if (err)
              return cb(err);
            if (stat.isDirectory())
              return cb(null);
            return cb(new Error("Cannot create " + made + " (in " + path + ") because it exists and is not a directory", "ENOENT"));
          });
        });
      },
      cb);
...
function mkParentPath(path, cb) {
  return mkpath(path.replace(/\/[^/]*$/, ''), cb);
};
