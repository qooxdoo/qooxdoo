...
Imagine you are writing a complex form with multiple widgets containing other widgets.
This complex nesting becomes difficult to manage due to having to instantiate the widgets and sub-widgets in the correct order.
...
qx.Class.define("qx.test.MyForm", {
  ...
  construct() {
    ...
    let compTop = new qx.ui.container.Composite(new qx.ui.layout.VBox());
    ...
    let compBottom = new qx.ui.container.Composite(new qx.ui.layout.VBox());
    
    let sp = new qx.ui.form.TextField();
    compBottom.add(sp);

    let sp1 = new qx.ui.form.TextField();
    compBottom.add(sp1);

    this._add(compBottom);
  }
})
```
...
qx.Class.define("qx.test.MyForm", {
  ...
  construct() {
    ...
    //We call the method 'getQxObject' to reference a cached object
    //When the object with the id is referenced initially, its generator function will run, store the result in a cache and return the object
    //However, when called subsequently, the cached value will be returned
    this._add(this.getQxObject("compTop"));
    this._add(this.getQxObject("compBottom"));


    //This is the second time we are referencing "compBottom", so it will be retrieved from the cache
  ...
  objects: {
    //We name the generator functions after the cached object's id
    compTop() {
...
We can store any kind of value we want as a cached object; we are not just restricted to widgets. 
