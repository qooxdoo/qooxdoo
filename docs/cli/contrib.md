...
generated by semver strictly.

### Contribution dependency management, a note on NPM

From the perspective of the compiler, contribs are qooxdoo libraries that have a
peer-dependency on the qooxdoo version used by the compiler to create the final
build. That is, the contribs cannot specify which version of qooxdoo they will
be compiled with, they can only state (via the `qooxdoo-range` key in
`Manifest.json`) which versions they are compatible with. That is, a compatible
qooxdoo version, and, in addition, a compatible compiler version, are a
prerequisites*, not *dependencies* of a contrib and will be described and
validated via `Manifest.json` (TBD).
...
A note on NPM: Under normal circumstances, a contrib does not need to use NPM or
maintain a `package.json` file. In particular, neither the `qxcompiler` nor
the `qooxdoo-sdk` npm packages should be NPM dependencies of the project, since
you should always install them globally (qooxdoo-ssk is automatically installed
by qxcompiler). You might want to use NPM for development-time task such as 
transpiling your code, but all NPM-related information will be ignored by qooxdoo.
