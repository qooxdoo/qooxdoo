...
`requires.@qooxdoo/framework`. The `qx contrib` commands handle the compatibility data
...
For example, a contrib that depends on `"@qooxdoo/framework":"^6.0.0-alpha"` will be 
...
the `@qooxdoo/framework` npm packages should be NPM dependencies of the contrib.
