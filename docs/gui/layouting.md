...
To make layout properties available in a convenient fashion each [add()](http://www.qooxdoo.org/devel/api/#qx.ui.container.Composite~add) has an optional second parameter: A map with all layout properties to configure. A basic example:
...
-   [TabView Page](http://www.qooxdoo.org/devel/api/#qx.ui.tabview.Page)
-   [Popup](http://www.qooxdoo.org/devel/api/#qx.ui.popup.Popup)
...
-   [Window](http://www.qooxdoo.org/devel/api/#qx.ui.window.Window)
-   [GroupBox](http://www.qooxdoo.org/devel/api/#qx.ui.groupbox.GroupBox)
...
In the first case we often see that an application reduces the size of the label and uses an ellipsis symbol to show that the label was too long. This feature is built-in into both commonly used widgets: [Label](http://www.qooxdoo.org/devel/api/#qx.ui.basic.Label) and [Atom](http://www.qooxdoo.org/devel/api/#qx.ui.basic.Atom). When the underlying layout ask to reduce the width (or the developer using the `width` property) the widget tries to solve the requirement dynamically. This certainly works for the height as well.
...
The second case is handled by the [height for width](http://www.qooxdoo.org/devel/api/#qx.ui.core.LayoutItem~_getHeightForWidth) support. Lengthy name but basically a really strong feature which is required quite often. It means that the height may depend on the actual width available. This especially makes sense for multi-line text where the wrapping may be influenced by the available width. The [Label](http://www.qooxdoo.org/devel/api/#qx.ui.basic.Label) widget includes support for this feature when using the [rich](http://www.qooxdoo.org/devel/api/#qx.ui.basic.Label~rich) output mode (HTML content).
...
Finally this means that every widget can grow and shrink depending on the limitations given for the respective axis. Two easy accessors which disable growing or shrinking respectively are [allowGrowX](http://www.qooxdoo.org/devel/api/#qx.ui.core.LayoutItem~allowGrowX) and [allowShrinkX](http://www.qooxdoo.org/devel/api/#qx.ui.core.LayoutItem~allowShrinkX). When the growing is disabled the configured or automatically detected maximum size is ignored and configured to the preferred size. When the shrinking is disabled the configured or automatically detected minimum size is ignored and configured to the preferred size. Two convenient methods to controlling these features without knowing of the exact dimensions.

Overflow Handling
-----------------

This leads to the next question: how to handle scenarios where the content needs more room than provided by the parent but should not shrink. This is a common case for data widgets like [Lists](http://www.qooxdoo.org/devel/api/#qx.ui.form.List) or [Trees](http://www.qooxdoo.org/devel/api/#qx.ui.tree.Tree). Both extend the [AbstractScrollArea](http://www.qooxdoo.org/devel/api/#qx.ui.core.scroll.AbstractScrollArea) to provide scrollbars to handle overflowing content.
...
The scrollbars are [controlable in a way that is comparable to CSS](http://www.qooxdoo.org/devel/api/#qx.ui.core.scroll.AbstractScrollArea~scrollbarX). It is possible to have both scrollbars marked as `auto` to automatically detect the needs of the content. Or any other combination where a scrollbar may be statically hidden or visible. Each bar can be controlled separately. It is possible to enable one scrollbar statically and make the other one auto-displayed and vice-versa.
...
The `ScrollArea` provides all typically needed methods like [scrollToX](http://www.qooxdoo.org/devel/api/#qx.ui.core.scroll.AbstractScrollArea~scrollToX) to scroll to an absolute position or [scrollByX](http://www.qooxdoo.org/devel/api/#qx.ui.core.scroll.AbstractScrollArea~scrollByX) to scroll by the given amount. The widget also supports the scrolling of any child into the viewport. This feature is provided through the method [scrollItemIntoView](http://www.qooxdoo.org/devel/api/#qx.ui.core.Widget~scrollChildIntoView). It just needs any child of the widget (at any depth).
...
The `flex` property is supported by both [Box Layouts](http://www.qooxdoo.org/devel/api/#qx.ui.layout.HBox), the [Dock](http://www.qooxdoo.org/devel/api/#qx.ui.layout.Dock) Layout and the [Grid](http://www.qooxdoo.org/devel/api/#qx.ui.layout.Grid) (for columns and rows).

In some way the [SplitPane](http://www.qooxdoo.org/devel/api/#qx.ui.splitpane) supports flex as well, but it behaves a bit different there as it is regarded as an alternative to the preferred size.
...
The effects of percentage dimensions in box layouts are comparable to the result of flex in a [SplitPane](http://www.qooxdoo.org/devel/api/#qx.ui.splitpane). The resulting size is computed from the available space less all statically configured gaps like spacings or margins. Layout managers with support for percentage dimensions are the already mentioned [Box](http://www.qooxdoo.org/devel/api/#qx.ui.layout.VBox) Layouts, but also the [Canvas](http://www.qooxdoo.org/devel/api/#qx.ui.layout.Canvas) Layout as well as the [Dock](http://www.qooxdoo.org/devel/api/#qx.ui.layout.Dock) Layout.
