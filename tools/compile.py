      ...
      if mode == "js":
        ...
        if s_klammerzu:
          if line[0:4] != "else" and line[0:5] != "while" and line[0:5] != "catch":
            print "    * Warning: Missing semicolon after fragment: %s" % s_out[-30:]
            s_out += ";"
...
def main():
  ...
  print "  - Public Path: %s" % settings.publicpath
  ...
  for root, dirs, files in os.walk(settings.searchpath):
    ...
    for file in files:
      ...
      mkdir(os.getcwd() + os.sep + settings.publicpath + os.sep + dirout)

      cachefile = settings.cachepath + os.sep + dirout + file
      outfile = settings.publicpath + os.sep + dirout + file
  ...
  if settings.job == "all" or settings.job == "doc":
    for mode in data.keys():
      ...
      os.system("cp -af " + settings.relpath + "/style " + os.getcwd() + os.sep + settings.docpath)
      os.system("cp -af " + settings.relpath + "/script " + os.getcwd() + os.sep + settings.docpath)
